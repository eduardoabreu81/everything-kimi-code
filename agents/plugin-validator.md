...
1. **Locate Plugin Root**:
   - Check for `.claude-plugin/plugin.json` (Claude format) or `.kimi/` directory (Kimi format)
   - Verify plugin directory structure
   - Note plugin location (project vs marketplace)

2. **Validate Manifest** (`.claude-plugin/plugin.json` for Claude, `.kimi/config.toml` for Kimi):
   - Check JSON syntax (use Shell with `jq` or ReadFile + manual parsing)
...
5. **Validate Agents** (if `agents/` exists):
   ...
   - For each agent file:
     - Check frontmatter with `name`, `description`
     - Name format (lowercase, hyphens, 3-50 chars)
     - Description includes triggering examples
     - System prompt exists and is substantial (>20 chars)
     - For Kimi format: verify companion `.yaml` file exists
