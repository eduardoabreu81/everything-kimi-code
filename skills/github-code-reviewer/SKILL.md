...
```mermaid
flowchart TD
    A([BEGIN]) --> B[Check Eligibility - Verify gh CLI is installed and authenticated]
    B --> C{Eligible?}
    ...
    H --> I{Score >= 80?}
    I -->|Yes| J[Post Comment - Comment approval on PR]
    I -->|No| K[Post Comment - Comment with issues and score]
    J --> L([END])
    K --> L
```
